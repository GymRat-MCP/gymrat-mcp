...
- 모든 description은 서비스명 'GymRat'를 포함한다.
"""
import os
...
def get_profile(user_id: str) -> dict:
    """GymRat: 사용자의 저장된 PT 프로필(목표·경력·부상·페르소나)을 조회한다."""
...
def update_profile(user_id: str, goal: str | None = None,
    ...
    """GymRat: 프로필을 생성/부분 갱신한다(전달된 필드만).
...
def log_inbody(user_id: str, weight: float | None = None,
    ...
    """GymRat: 사용자가 알려준 인바디 측정 수치(체중·골격근량·체지방률)를 저장하고 프로필에 반영한다.
    응답에 assistant_message가 있으면 사용자에게 이 문장을 우선 전달한다."""
...
def edit_workout(user_id: str, log_id: int,
                 ...
                 raw_text: str = None,
                 date: str = None) -> dict:
    """GymRat: 기존 운동 기록을 수정한다(오타 교정·날짜 정정).
...
def delete_workout(user_id: str, log_id: int) -> dict:
    """GymRat: 잘못 남긴 운동 기록을 삭제한다(연결된 세트도 함께 제거, 통계 반영).
...
def get_recent_workouts(user_id: str, limit: int = 10) -> dict:
    """GymRat: 최근 운동 기록을 log_id와 함께 조회한다(수정·삭제 대상 식별용).
...
def log_meal(user_id: str, meal_text: str,
    ...
    """GymRat: 식단 텍스트를 저장하고 질적 코멘트를 단다(수치 처방 X).
...
def confirm_meal_details(
    ...
    """GymRat: 애매한 식단 기록의 보충 답변을 받아 최종 저장한다.
...
def analyze_trend(user_id: str, metric: str = "weight",
    ...
    """GymRat: 추세 분석. metric: weight|volume|inbody|meal.
    응답에 assistant_message가 있으면 사용자에게 이 문장을 우선 전달한다."""
...
def get_exercise_history(user_id: str, exercise: str,
    ...
    """GymRat: 특정 종목의 힘 추세를 보여준다("내 벤치가 는다"를 숫자로).
...
def get_weekly_recap(user_id: str, week_offset: int = 0) -> dict:
    """GymRat: 주간 운동 리캡 — 세션수·총 볼륨·부위별 볼륨·지난주 대비 변화·신규 PR·
...
def get_goal_projection(user_id: str, exercise: str, target_weight: float,
    ...
    """GymRat: 목표 무게 도달 예상일을 투영한다("9월까지 벤치 100kg").
...
def generate_routine(user_id: str, focus: str | None = None,
    ...
    """GymRat: 목표·이력 기반 운동 루틴을 처방한다(점진적 과부하 + 자세 큐 + 부상 회피).
...
def set_workout_split(user_id: str, days: list[dict]) -> dict:
    """GymRat: 유저의 커스텀 운동 분할을 저장한다(중급+가 자기 분할을 직접 정의).
...
def get_workout_split(user_id: str) -> dict:
    """GymRat: 현재 유효 분할을 조회한다 — 커스텀이 있으면 커스텀, 없으면 프리셋(스타일·일수 해석).
...
def generate_meal_plan(user_id: str, goal_override: str | None = None,
    ...
    """GymRat: 다양한 균형 식단 후보에서 선호·비선호·알레르기를 반영해 끼니별로 추천한다.
...
def suggest_meal_adjustment(
    ...
    """GymRat: 현재 식사·최근 식단·몸 정보·운동량을 묶어 질적 식단 조정을 제안한다.
...
def get_recommendation(
    ...
    """GymRat: 식단·추세·방향성을 종합한 질적 코칭 한 마디.
