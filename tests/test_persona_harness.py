...
class PersonaHarnessTest(unittest.TestCase):
    ...
    def test_persona_rendering_is_deterministic_without_canned_suffixes(self):
        removed_canned_suffixes = {
        ...
        for persona in SUPPORTED_PERSONAS:
            with self.subTest(persona=persona):
                ...
                rendered = apply_persona("단백질을 보강해봐요.", persona)
                self.assertFalse(any(
                    rendered.endswith(suffix)
                    for suffix in removed_canned_suffixes
                ), rendered)
